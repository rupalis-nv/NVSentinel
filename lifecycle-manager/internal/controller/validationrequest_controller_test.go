...
var _ = Describe("ValidationRequest Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind ValidationRequest")
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance ValidationRequest")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &ValidationRequestReconciler{
				Client: k8sClient,
