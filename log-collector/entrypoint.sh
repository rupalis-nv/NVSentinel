...
# Skip uploads in mock mode (file server may not be deployed in test environments)
if [ -n "${UPLOAD_URL_BASE:-}" ] && [ "${MOCK_MODE}" != "true" ]; then
  ...
  echo "[MOCK] Skipping upload in mock mode - artifacts created locally at ${ARTIFACTS_DIR}"
fi
