...
* **providers:** includes test provider settings that apply to all tests using this provider.
    * **apiGroup / version / kind:** the GroupVersionKind of the provider CRD. The controller uses this to register a watch for each provider type so that test provider resource events trigger ValidationRequest reconciliation. Additionally, this information is used to create, get, and delete provider resources during reconciliation.
    * **resource:** the plural resource name of the provider CRD. This is only used to generate the validation-controller's RBAC rules via Helm templating.
    ...
    * **timeoutSeconds:** the maximum number of seconds allowed for a single test group attempt using this provider before it is marked as failed. If the provider itself enforces a timeout (for example, K8s Jobs support activeDeadlineSeconds), this value is also passed to the provider resource via the template.
...
* **env:** a list of environment variables with names and values to set in the provider resource template for a given test.
...
* **minimumNodesPerBatch:** if supportsBatchingNodes is true, this setting indicates the minimum number of nodes that must be provided to the test.
    ...
    * If supportsBatchingNodes is false, minimumNodesPerBatch must be 1. Each test group only ever covers 1 node when node batching is unsupported, so any value greater than 1 could never be satisfied.
...
The template files referenced by each provider are mounted into the controller from a validation-templates ConfigMap. Note that metadata.name and metadata.namespace in the rendered output are always overwritten by the controller after rendering, regardless of what the template specifies.
            ...
            - name: test
              ...
              env:
                - name: NVIDIA_GPUS_VALIDATED
                  value: {{quote (join (index .Nodes 0).GPUUUIDs ",")}}
                {{- range .Env}}
                - name: {{.Name}}
                  value: {{quote .Value}}
                {{- end}}
...
A validating webhook will enforce the following properties in ValidationRequests. This will prevent a validation session from starting against the targeted nodes, which is preferable to accepting the ValidationRequest and immediately marking it as failed.
...
7. Nodes and tests are immutable after creation.
...
metadata:
  annotations:
    nvsentinel.nvidia.com/validation-session: |
      [{"name":"validation-request-1","gpuUUIDs":["GPU1"],"tests":["dcgm-level4","nccl-loopback","cuda-smoke-test"]}]

# Node2
metadata:
  annotations:
    nvsentinel.nvidia.com/validation-session: |
      [{"name":"validation-request-1","gpuUUIDs":[],"tests":["dcgm-level4","nccl-loopback","cuda-smoke-test"]}]

# Node3
metadata:
  annotations:
    nvsentinel.nvidia.com/validation-session: |
      [{"name":"validation-request-1","gpuUUIDs":["GPU1","GPU2"],"tests":["dcgm-level4","nccl-loopback","cuda-smoke-test"]}]
```
...
* **Blocked ValidationRequests:** ValidationRequests can remain in a pending state if a node referenced in the request already has an in-progress ValidationRequest or a node does not meet its readinessCriteria. Tests that do not meet their batch minimums when the initial test groups are built are not blocking and instead defer immediately to their batchFailurePolicy (fail or ignore). When the policy is fail at this initial build step, the test group is marked as failed with a BatchMinimumNotMet failure reason and no retry is attempted. A test group can also later drop below its batch minimum if a node is deleted, in which case the group fails with either a NodeDeleted failure reason or, if no attempt had started yet, no attempt at all.
...
* **Test group failure reasons:** each test group attempt tracks a failureReason of TestFailed, TestTimeout, NodeReadinessViolation, NodeDeleted, or BatchMinimumNotMet. A NodeReadinessViolation occurs if any node in the test group violates its readinessCriteria after the test started. In this case, the test provider CRD is immediately deleted and the test group is retried once the node satisfies its readiness criteria again. A NodeDeleted failure occurs if any node in the test group no longer exists once the group already has an attempt in progress. The deleted node is removed from the group, and if this drops the group below its batch minimum, the group fails with a NodeDeleted failure reason (not BatchMinimumNotMet). A BatchMinimumNotMet failure occurs only when the initial test groups are built and a test cannot satisfy its minimumGPUsPerNodePerBatch or minimumNodesPerBatch requirements. In this case, no test provider CRD is created and the test group is immediately marked as failed without retrying, since the nodes and GPUs specified in the ValidationRequest will not change between retries. A ValidationRequest is marked as failed once any test group exhausts all its provider retries or receives a terminal BatchMinimumNotMet failure.
...
* **Test provider CRD deletion:** test provider CRDs are deleted by the controller on test group completion, regardless of whether the attempt succeeded or failed (TestFailed, TestTimeout, NodeReadinessViolation, or NodeDeleted). BatchMinimumNotMet failures do not create a test provider CRD and therefore require no cleanup. In addition, since test provider CRDs are owned by the ValidationRequest via OwnerReference, they will also be garbage collected if the ValidationRequest is deleted.
* **Test provider resource namespace:** all test provider resources, regardless of provider type, are created in the same namespace as the validation-controller's pod. Cluster-scoped test provider resources are not supported, and will be added as a follow-up.
...
* **Identifying failed nodes:** each attempt within a test group includes a failedNodes field that gives per-attempt visibility into failed nodes, since different retries may fail due to different nodes. Initially, failedNodes is only populated for the NodeReadinessViolation and NodeDeleted failure reasons, as the controller can directly identify which node violated its readiness criteria or no longer exists. As a follow-up, we could add support to consume failed node information directly from test providers for TestFailed and TestTimeout reasons.
