...
The `--wait-for-jobs` flag makes Helm wait for the MongoDB init Job (`-l app.kubernetes.io/name=create-mongodb-database`) as well; `--wait` alone only waits for the workloads.
