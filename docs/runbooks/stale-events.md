...
If this happens often, the oplog is too small for the write rate and the length of outages. Helm changing `mongodb-store.mongodb.persistence.size` (or the Percona rs0 PVC) does **not** expand an already-Bound volume. Expand and verify the live PVC with the backend’s supported procedure, then raise `mongodb-store.oplogSizeMB` (integer MiB; keep it under about half the live volume). The init Job will not shrink a larger live oplog unless `mongodb-store.oplogAllowShrink` is true. See [MongoDB Store Configuration](../configuration/mongodb-store.md#oplog-size).
